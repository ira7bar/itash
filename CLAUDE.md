...
- **A word's actual last letter is a deliberate exception to "nearest."** Typing it doesn't move the active cell at all (there's nowhere left to advance to), so plain `"nearest"` panning — which only acts when something isn't already in view — silently does nothing there, meaning the "peek past the edge" buffer would never get a chance to confirm the word really ended (board edge, or another definition). `interaction.js`'s `hiddenInput` "input" handler detects this case (cursor stayed on the just-written cell *and* whole-word mode is active) and uses `"center"` instead, the same deliberate scroll used for clue-tap entry, forcing that confirmation to happen right when it matters.
